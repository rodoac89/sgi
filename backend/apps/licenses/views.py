...
def forms_view(request):
    ...
    if request.GET:
        if 'id_user' in request.GET:
            data_forms = software_form.objects.get(id_request=int(request.GET['id_user']))
            context['formulario'] = data_forms

    return render(request, "solicitudes_software.html", context)

def form_list(request):
    software_forms = software_form.objects.all()
    data = {
...
def adm_licencias(request):
    licenses_lista = licenses_list.objects.all()
    data = {
